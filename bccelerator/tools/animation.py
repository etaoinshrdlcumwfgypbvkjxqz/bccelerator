...
def _copy_nla_strip(
    to_strip: _bpy.types.NlaStrip, from_strip: _bpy.types.NlaStrip
) -> None:
    attr: str
    for attr in (
        "action",
...
def _copy_nla_track(
    to_track: _bpy.types.NlaTrack, from_track: _bpy.types.NlaTrack
) -> None:
    attr: str
    for attr in (
        # 'active',
...
class CopySelectedNLATrack(_bpy.types.Operator):
    ...
    bl_idname: _typing.ClassVar[str] = "nla.copy_selected_track"  # type: ignore
    bl_label: _typing.ClassVar[str] = "Copy Selected NLA Track(s)"  # type: ignore
        ...
        for obj in context.selected_objects:
            if obj is not from_track.id_data:
                to_track: _bpy.types.NlaTrack = _util.intersection2(
                    _util_utils.ensure_animation_data(obj).nla_tracks
                )[0].new()
...
class RandomizeSelectedNLAStrip(_bpy.types.Operator):
    ...
    __slots__: _typing.ClassVar = ()
    bl_idname: _typing.ClassVar[str] = "nla.randomize_selected_strip"  # type: ignore
    bl_label: _typing.ClassVar[str] = "Randomize Selected NLA Strip(s)"  # type: ignore
    bl_options: _typing.ClassVar[  # type: ignore
        _typing.AbstractSet[_util_enums.OperatorTypeFlag]
    ] = frozenset(
        {_util_enums.OperatorTypeFlag.REGISTER, _util_enums.OperatorTypeFlag.UNDO}
    ...
    def execute(  # type: ignore
        ...
        for nla_strip in context.selected_nla_strips:
            nla_tracks: _typing.Iterator[_bpy.types.NlaTrack] = (
                track
                for track in _util.intersection2(
                    _util_utils.ensure_animation_data(
                        _typing.cast(_bpy.types.ID, nla_strip.id_data)
                    ).nla_tracks
                )[1]
                if nla_strip
                in (_util.intersection2(track.strips)[1].values)()  # type: ignore
            )
            try:
            ...
            if nla_track.lock:
                ...
                continue
            index: int = _util.intersection2(nla_track.strips)[1].find(nla_strip.name)
            if index == -1:
                self.report(
                    {_util_enums.WMReport.WARNING},
            ...
            start_max: int = int(
                (
                    ...
                    < len(
                        _typing.cast(
                            _typing.Collection[_bpy.types.NlaStrip], nla_track.strips
                        )
                    )
