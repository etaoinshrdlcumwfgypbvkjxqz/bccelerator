...
class CleanUpLibraryWeakReference(_bpy.types.Operator):
    ...
    bl_label: _typing.ClassVar[str] = (  # type: ignore
        'Clean Up Library Weak References'
    )
    bl_options: _typing.ClassVar[_typing.AbstractSet[_util_enums.OperatorTypeFlag]] = (  # type: ignore
        frozenset({
            _util_enums.OperatorTypeFlag.REGISTER, _util_enums.OperatorTypeFlag.UNDO, })
    ...
    def OUTLINER_MT_context_menu_draw_func(cls: type[_util_polyfill.Self], self: _typing.Any, context: _bpy.types.Context) -> None:
        ...
        if RemapUserToLibraryByName.poll(context):
            lambdas.append(lambda:
                           layout.operator(RemapUserToLibraryByName.bl_idname))
        if RemapUserToLocalByName.poll(context):
            lambdas.append(lambda:
                           layout.operator(RemapUserToLocalByName.bl_idname))
        if LocalizeLibrary.poll(context):
            lambdas.append(lambda:
                           layout.operator(LocalizeLibrary.bl_idname))
        if lambdas:
            layout.separator()
