...
def draw_func_class(cls: type[_T]) -> type[_T]:
    ...
    registry: _typing.MutableMapping[str, _typing.Callable[[
...
def internal_operator(*, uuid: str) -> _typing.Callable[[type[_T]], type[_T]]:
    def decorator(cls: type[_T]) -> type[_T]:
        ...
        setattr(cls, 'bl_options', frozenset(
            {_enums.OperatorTypeFlag.INTERNAL, }))

        @classmethod
        def poll(cls: type[_T],
                 context: _bpy.types.Context) -> bool:
            return False
