...
import bpy as _bpy, texts.common as _common
	...
	def execute(self, context):
		...
		if 'Base Color' in node.inputs\
			...
			for (index, curve) in enumerate(curves):
				_common.utils.configure_driver(curve.driver,
					...
					data_path=
					'nodes["%s"].inputs["Base Color"].default_value[%d]'
					% (node.name, index),
				)
				curve.lock = True
		...
		if 'Metallic' in node.inputs\
			...
			_common.utils.configure_driver(curve.driver,
				id_type='NODETREE', id=node_tree,
				data_path=
				'nodes["%s"].inputs["Metallic"].default_value'
				% node.name,
			)
			curve.lock = True
			processed += 1
		...
		if 'Roughness' in node.inputs\
			...
			_common.utils.configure_driver(curve.driver,
				id_type='NODETREE', id=node_tree,
				data_path=
				'nodes["%s"].inputs["Roughness"].default_value'
				% node.name,
			)
			curve.lock = True
			processed += 1
		...
		if 'Alpha' in node.inputs:
			if material.blend_method == 'OPAQUE'\
				...
				_common.utils.configure_driver(curve.driver,
					id_type='NODETREE', id=node_tree,
					data_path=
					'nodes["%s"].inputs["Alpha"].default_value'
					% node.name,
